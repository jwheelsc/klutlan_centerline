...
%         plot(sf(:,1),sf(:,2),'b')
%     end
    savePDFfunction(f1,['F:\surge_project\code\figures\pairs_' pnum '\strain_' pnum '\' fName(38:66)])
...
dvpSs = dvpS(:,els)
vpAs = vpA(:,els)
save(['strain_' pnum '.mat'],'dvpSs','vpAs','namArr')
%%
close all
f2 = figure
...
set(gca,'fontsize',18)
title(['Steele glacier dwdz from ' pnum ' day pairs'])
savePDFfunction(f2,['F:\surge_project\code\figures\pairs' pnum '\strain_' pnum])
...
for i = 1:length(keep_i)
    ...
    savePDFfunction(f3,['F:\surge_project\code\figures\pairs_' pnum '\strain_' pnum '_dt\' fName(38:66)])
%     pause(timeArr(i))
